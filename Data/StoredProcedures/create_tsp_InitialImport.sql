IF OBJECT_ID(N'tsp_InitialImport', N'P') IS NOT NULL
  DROP PROCEDURE tsp_InitialImport;
PRINT 'IS NOT NULL'
GO
CREATE PROCEDURE [dbo].[tsp_InitialImport] AS
	SET NOCOUNT ON;  
	...
	USING (SELECT
		...
		SUM(_Fld16305),
		1
	FROM homnet_fin.dbo._InfoRg16300
	JOIN homnet_2.dbo._InfoRg14815 ON _Fld14841 = _Fld16304
	JOIN upr_ulf_finance.dbo._InfoRg7874
	ON(CONVERT(INT, _Fld7893) = CONVERT(INT, _Fld14836) AND CONVERT(INT, _Fld7893) > 0) OR LTRIM(RTRIM(_Fld14820)) = LTRIM(RTRIM(_Fld7877))
	JOIN Agreements ON UprId = _Fld7875
	WHERE _Fld16305 < 0
	GROUP BY Id, _Fld16302) AS source (AgreementId, Date, Amount, Currency)  
	...
	WHEN MATCHED THEN
		UPDATE SET Amount = source.Amount
	WHEN NOT MATCHED THEN  
		INSERT (AgreementId, Date, Amount, Currency, Source)
		VALUES (source.AgreementId, source.Date, source.Amount, source.Currency, 0);
	GO